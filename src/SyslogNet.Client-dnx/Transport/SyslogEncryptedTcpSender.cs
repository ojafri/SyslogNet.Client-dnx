...
namespace SyslogNet.Client.Transport
{
	public class SyslogEncryptedTcpSender : SyslogTcpSender
	{
        ...
        public X509CertificateCollection CertificateCollection { get; private set; }
        ...
        public SyslogEncryptedTcpSender(string hostname, int port, X509Certificate certificate, int timeout = Timeout.Infinite, bool ignoreChainErrors = false) : base(hostname, port)
        {
            IOTimeout = timeout;
            IgnoreTLSChainErrors = ignoreChainErrors;
            this.CertificateCollection = new X509CertificateCollection { certificate };
            startTLS();
        }

        public SyslogEncryptedTcpSender(string hostname, int port, X509CertificateCollection certificateCollection, int timeout = Timeout.Infinite, bool ignoreChainErrors = false) : base(hostname, port)
		{
			IOTimeout = timeout;
			IgnoreTLSChainErrors = ignoreChainErrors;
